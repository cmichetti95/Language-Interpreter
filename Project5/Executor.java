...
class Executor {
	...
	// Creating an array list to hold our reference counts.
	...
	static void initialize(String dataFileName) {
		...
		// Also have to initialize the arraylist for refCounts now
	...
	static void popLocalScope() {
		...
		// now need to catch the hashmap that gets popped to check for variables going
		// out of scope.
		HashMap<String, CoreVar> temp = stackSpace.peek().pop();
		checkForClassVars(temp);
	}
	...
	static void heapAllocate(String identifier) {
		...
		// Since a new heap slot is being allocated, we need to also create a new slot
		// in our array list because there's a new reference.
		refCounts.add(1);
		printGC();
	}
	...
	static void referenceCopy(String var1, String var2) {
		...
		// checking if x exists in the heapspace already and decrementing if so.
		if (x.value != null) {
			int temp = refCounts.get(x.value);
			refCounts.set(x.value, temp - 1);
			
			//if the ref count is now 0, we need to print out the state of the GC
			if(refCounts.get(x.value) == 0) {
				printGC();
			}
		}
		x.value = y.value;

		// increment y because it now has a new reference.
		if (y.value != null) {
			int temp = refCounts.get(y.value);
			refCounts.set(y.value, temp + 1);
		}
	}
	...
	static void pushFrame(Formals formalParams, Formals actualParams) {
		...
		for (int i = 0; i < formals.size(); i++) {
			CoreVar temp = new CoreVar(Core.CLASS);
			...
			// need to increment because formals are adding new references
			int tempTwo = refCounts.get(temp.value);
			refCounts.set(temp.value, tempTwo + 1);
		}
	...
	static void popFrame() {
		...
		// now need to catch the stack of hashmaps for any variables going out of scope
		Stack<HashMap<String, CoreVar>> temp = stackSpace.pop();
		while (temp.size() > 0) {
			checkForClassVars(temp.peek());
			temp.pop();
		}
	}

	static void checkForClassVars(HashMap<String, CoreVar> temp) {
		for (CoreVar t : temp.values()) {
			if (t.type == Core.CLASS && t.value != null) {		
				int tempTwo = refCounts.get(t.value);
				refCounts.set(t.value, tempTwo - 1);
				
				//if the ref count is now 0, we need to print out the state of the GC
				if(refCounts.get(t.value)== 0) {
					printGC();
				}
			}
		}
	...
	// A print function which can print out our total ref count when called
	static void printGC() {
		int check = 0;
	...
	// Clears any remaining reference counts.
	static void clearStatic() {
		for (int i = 0; i < refCounts.size(); i++) {
			int temp = refCounts.get(i);
			
			//checking if there are any global variables left.
			if(temp != 0) {
				refCounts.set(i, 0);
				printGC();
			}
